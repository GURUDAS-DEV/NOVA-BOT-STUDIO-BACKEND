...
* Logs are output via `morgan` (HTTP) and `winston` (application) – adjust log level in `.env` (`LOG_LEVEL=debug`).
...
* **Heroku / Render** – set the buildpack to `heroku/nodejs` and provide the same environment variables.  
* **AWS ECS / Fargate** – push the Docker image to ECR and reference it in your task definition.  
* **DigitalOcean App Platform** – select “Dockerfile” as the source and add the required env vars.

### Performance considerations  

* Enable **Redis** clustering for high‑throughput API‑key validation.  
* Use **PostgreSQL connection pooling** (`pg-pool`) – adjust `MAX_POOL_SIZE` in `.env`.  
* Set `SESSION_TTL_SECONDS` according to your security policy; shorter TTL reduces stale session risk.

---  

## API Documentation  

All endpoints are prefixed with `/api`. The API follows REST conventions and returns JSON. Authentication is required for all routes except `/auth/register`, `/auth/login`, and `/ping`.
...
| `GET` | `/api/analytics/:botId` | ✅ | Get aggregated analytics for a bot |
| `POST` | `/api/ai-feature/toggle` | ✅ | Enable/disable AI modules per bot |
| `GET` | `/api/ai