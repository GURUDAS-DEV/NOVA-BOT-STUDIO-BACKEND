...
const toMinimalNode = (node: any) => {
    if (!node) return null;
    return {
        id: node?._id?.toString?.() || node?._id || node?.id,
        title: node?.title,
        ...
        output: node?.output ? {
            ...
            allowGoBack: node?.output?.allowGoBack,
            allowEnd: node?.output?.allowEnd,
            customText: node?.output?.customText || node?.output?.text || undefined,
        } : undefined,
        inputConfig: node?.inputConfig ? {
            key: node?.inputConfig?.key,
            validationRegex: node?.inputConfig?.validationRegex,
            retryLimit: node?.inputConfig?.retryLimit,
        } : undefined,
    };
};

const toMinimalOptions = (options: any[] = []) => {
    return options.map((option: any) => ({
        optionId: option?.optionId || option?._id,
...
const toMinimalNodeWithNavOptions = (node: any) => {
    if (!node) return null;
    return {
        node: toMinimalNode(node),
        options: getNavigationOptions(),
    };
};

const getNodeMessage = (node: any) => node?.output?.customText || node?.output?.text;

const withChatSessionId = (res: Response, payload: Record<string, any>) => {
    const chatSessionId = res.locals?.chatSessionId as string | undefined;
    return chatSessionId ? { ...payload, chatSessionId } : payload;
};
...
export const controlledStyleWebsiteBotController = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        let chatSessionId = req.get("chatSessionId") as string | undefined;
        console.log("Received chatSessionId from client:", chatSessionId);
        if (!chatSessionId || chatSessionId.trim() === "") {
            console.log("Generating new chatSessionId");
            chatSessionId = crypto.randomUUID();
            // Send session id only on the initial request so the client can store it.
            res.locals.chatSessionId = chatSessionId;
        }
        ...
        // Helper to set session and return common responses
        const setSessionAndReturn = async (opts: {
            type: 'options' | 'text' | 'input' | 'api',
            nextNodeId?: any | null,
            previousNodeId?: any | null,
            nodeDataContainer?: any,
            nodeDataObj?: any,
            message?: any,
        }) => {
            const { type, nextNodeId, previousNodeId, nodeDataContainer, nodeDataObj, message } = opts;
            try {
                await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nextNodeId ? nextNodeId.toString() : null, previousNodeId: previousNodeId ? previousNodeId.toString?.() ?? previousNodeId : previousNodeId }), { ex: 60 * 60 });
            } catch (err) { }

            if (type === 'options') {
                return res.status(200).json({ type: "options", nodeData: toMinimalNodeWithOptions(nodeDataContainer || nodeDataObj) });
            }

            if (type === 'text') {
                return res.status(200).json({ type: "text", message: message || getNodeMessage(nodeDataObj), nodeData: toMinimalNodeWithNavOptions(nodeDataObj), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
            }

            if (type === 'input') {
                return res.status(200).json({ type: "input", nodeData: toMinimalNodeWithNavOptions(nodeDataObj), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
            }

            if (type === 'api') {
                return res.status(200).json({ type: "api", message: message, nodeData: toMinimalNodeWithNavOptions(nodeDataObj), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
            }
        ...
        if (!sessionalBotDetails) {
            ...
            if (!initialNodeDataFromRedis) {
                ...
                return await processInitialNodeFromRedis(initialNodeDataFromRedis, initialNodeId.toString(), redisKeyForNodes, sessionalRedisKey, res);
            }
        } else {
            ...
            if (GO_BACK) {
                ...
                if (previousNodeDataFromRedis?.currentNode.executor === 'none' && previousNodeDataFromRedis?.currentNode?.output?.mode === 'options') {
                    ...
                    return res.status(200).json({ type: "options", nodeData: toMinimalNodeWithOptions(previousNodeDataFromRedis), nodeData1: toMinimalNodeWithOptions(previousNodeDataFromRedis) });
                    ...
                    const previousNodeData = previousNodeDataFromRedis?.currentNode || previousNodeDataFromRedis;
                    return res.status(200).json({ type: "text", message: getNodeMessage(previousNodeData), nodeData: toMinimalNodeWithNavOptions(previousNodeData), nodeData2: toMinimalNode(previousNodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
                    ...
                    const previousNodeData = previousNodeDataFromRedis?.currentNode || previousNodeDataFromRedis;
                    return res.status(200).json({ type: "input", nodeData: toMinimalNodeWithNavOptions(previousNodeData), nodeData3: toMinimalNode(previousNodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
                }
            }
            ...
            if (currentNodeData.executor === 'none' && currentNodeData.output?.mode === 'options') {
                ...
                const nextNodeDataContainer = nextNodeDataFromRedis;
                const nextNodeData = nextNodeDataFromRedis?.currentNode || nextNodeDataFromRedis;
                // Return next node
                if (nextNodeData?.executor === 'none' && nextNodeData?.output?.mode === 'options') {

                    return await setSessionAndReturn({ type: 'options', nextNodeId, previousNodeId: sessionCurrentNodeId, nodeDataContainer: nextNodeDataContainer });
                }
                else if (nextNodeData?.executor === 'none' && nextNodeData?.output?.mode === 'text') {
                    return await setSessionAndReturn({ type: 'text', nextNodeId, previousNodeId: sessionCurrentNodeId, nodeDataObj: nextNodeData });
                }
                else if (nextNodeData?.executor === 'input') {

                    return await setSessionAndReturn({ type: 'input', nextNodeId, previousNodeId: sessionCurrentNodeId, nodeDataObj: nextNodeData });
                }
                else if (nextNodeData?.executor === 'api' && nextNodeData?.output?.mode === 'text') {
                    const apiNode = (nextNodeData as any)?.currentNode || nextNodeData;
                    ...
                    return await setSessionAndReturn({ type: 'api', nextNodeId, previousNodeId: sessionCurrentNodeId, nodeDataObj: apiNode, message: apiData });
                }

                else if (nextNodeData?.executor === 'api' && nextNodeData?.output?.mode === 'options') {
                    const apiNode = (nextNodeData as any)?.currentNode || nextNodeData;
                    const reqInfo = extractApiRequest(apiNode?.apiConfig);
                    ...
                    return await setSessionAndReturn({ type: 'options', nextNodeId, previousNodeId: sessionCurrentNodeId, nodeDataContainer: nodeDataWithOptions });
                }
            }
                ...
                if (GO_BACK) {
                    ...
                    await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: previousNodeId.toString(), previousNodeId: null }), { ex: 60 * 60 });
                    return res.status(200).json({ type: "text", message: getNodeMessage(previousNodeDataFromRedis), nodeData: toMinimalNodeWithNavOptions(previousNodeDataFromRedis), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
                }
                ...
                return res.status(200).json({ type: "text", message: getNodeMessage(currentNodeData), nodeData: toMinimalNodeWithNavOptions(currentNodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } });
            }
                ...
                // Return next node
                if (nextNodeData?.executor === 'none' && nextNodeData?.output?.mode === 'options') {
                    return await setSessionAndReturn({ type: 'options', nextNodeId, previousNodeId: sessionalBotDetails.previousNodeId, nodeDataContainer: nextNodeDataContainer });
                }

                else if (nextNodeData?.executor === 'none' && nextNodeData?.output?.mode === 'text') {
                    return await setSessionAndReturn({ type: 'text', nextNodeId, previousNodeId: sessionalBotDetails.previousNodeId, nodeDataObj: nextNodeData });
                }

                else if (nextNodeData?.executor === 'input') {
                    return await setSessionAndReturn({ type: 'input', nextNodeId, previousNodeId: sessionalBotDetails.previousNodeId, nodeDataObj: nextNodeData });
                }

                else if (nextNodeData?.executor === 'api' && nextNodeData?.output?.mode === 'text') {

                    const apiFormatFromLLM: apiConstructorTemplate = await constructApiUponUserInput(nextNodeData.apiConfig, input);
                    ...
                    return await setSessionAndReturn({ type: 'api', nextNodeId, previousNodeId: sessionalBotDetails.previousNodeId, nodeDataObj: nextNodeData, message: responseToSend });
                }

                else if (nextNodeData?.executor === 'api' && nextNodeData?.output?.mode === 'options') {
                    const apiEndpoint = nextNodeData.apiConfig?.endpointKey;
                    if (!apiEndpoint) {
...
const processInitialNode = async (
    ...
    if (nodeData.executor === 'none' && nodeData.output?.mode === 'options') {
        ...
        return res.status(200).json(withChatSessionId(res, { type: "options", nodeData: toMinimalNodeWithOptions(nodeWithOptions) }));
        ...
        await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nodeId, previousNodeId: null }), { ex: 60 * 60 });
        return res.status(200).json(withChatSessionId(res, { type: "text", message: getNodeMessage(nodeData), nodeData: toMinimalNodeWithNavOptions(nodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } }));
    } else if (nodeData.executor === 'input') {
        await redis.set(redisKeyForNodes(nodeId), JSON.stringify(nodeData), { ex: 86400 });
        await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nodeId, previousNodeId: null }), { ex: 60 * 60 });
        return res.status(200).json(withChatSessionId(res, { type: "input", nodeData: toMinimalNodeWithNavOptions(nodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } }));
        ...
        return res.status(200).json(withChatSessionId(res, { type: "api", message: response, nodeData: toMinimalNodeWithNavOptions(nodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } }));
    }
        ...
        await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nodeId, previousNodeId: null }), { ex: 60 * 60 });
        return res.status(200).json(withChatSessionId(res, { type: "options", nodeData: toMinimalNodeWithOptions(nodeDataWithOptions) }));
    }
...
const processInitialNodeFromRedis = async (
    ...
    if (nodeData.executor === 'none' && nodeData.output?.mode === 'options') {
        await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nodeId, previousNodeId: null }), { ex: 60 * 60 });
        return res.status(200).json(withChatSessionId(res, { type: "options", nodeData: toMinimalNodeWithOptions(nodeDataFromRedis) }));
    } else if (nodeData.executor === 'none' && nodeData.output?.mode === 'text') {
        await redis.set(sessionalRedisKey, JSON.stringify({ currentNodeId: nodeId, previousNodeId: null }), { ex: 60 * 60 });
        return res.status(200).json(withChatSessionId(res, { type: "text", message: getNodeMessage(nodeData), nodeData: toMinimalNodeWithNavOptions(nodeData), options: getNavigationOptions(), back: { label: "Go Back", _id: "GO_BACK" }, end: { label: "End Chat", _id: "END_CHAT" } }));
    } else if (nodeData.executor === 'input') {
